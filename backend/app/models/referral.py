...
class ReferralOffer(SQLModel, table=True):
    ...
    tags: list[str] = Field(sa_column=Column(ARRAY(String)))  # role/skill areas, same vocabulary as JobPosting.tags
...
class ReferralRequest(SQLModel, table=True):
    ...
    posting_id: Optional[UUID] = Field(default=None, foreign_key="jobposting.id")
    match_score: Optional[float] = None   # tag-overlap ratio, 0-1
