...
def create_intel(session: Session, intel_in: InterviewIntelCreate, user_id: UUID) -> dict:
    ...
    return {**new_intel.model_dump(), "job_title": job.title, "company_name": company.name if company else "Unknown"}


def get_recent_intel(session: Session) -> list[dict]:
    results = session.exec(
        select(InterviewIntel, JobPosting, Company)
        .join(JobPosting, InterviewIntel.posting_id == JobPosting.id)
        .join(Company, InterviewIntel.company_id == Company.id)
        .order_by(InterviewIntel.created_at.desc())
        .limit(50)
    ).all()

    return [{**intel.model_dump(), "job_title": job.title, "company_name": company.name} for intel, job, company in results]
