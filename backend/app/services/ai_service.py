...
COMMON_KEYWORDS = [
    ...
    "node", "sql", "api", "git", "ci/cd", "frontend", "backend",
]
...
def _extract_text(file_bytes: bytes, filename: str) -> str:
    ...
    return text or "Resume text could not be extracted."

extract_text_from_file = _extract_text


def _fallback_analysis(job: JobPosting, resume_text: str) -> Dict[str, Any]:
    ...
    matching_keywords = [kw for kw in COMMON_KEYWORDS if kw in job_text and kw in resume_lower]
    all_job_keywords = [kw for kw in COMMON_KEYWORDS if kw in job_text]

    base_score = 65
    if tags:
        base_score += int((len(matching_tags) / len(tags)) * 25)
    elif all_job_keywords:
        base_score += int((len(matching_keywords) / len(all_job_keywords)) * 25)
    else:
        base_score += 15
    ...
    gaps = [t.capitalize() for t in missing_tags] or ["Advanced cloud deployment", "Specific toolchain specialization"]

    return {
        "match_score": match_score,
        "match_summary": f"Your profile shows a strong match ({match_score}%) for the {job.title} position.",
        "strengths": strengths[:4],
        "gaps": gaps[:3],
        "referral_pitch": (
            f"Hi! I came across the {job.title} opening and am very excited about the role. "
            f"With experience in {', '.join(strengths[:3])}, I believe I can make an immediate impact. "
            f"Would you be open to providing a referral for my application?"
        ),
    }


def _call_gemini(api_key: str, job: JobPosting, resume_text: str) -> Dict[str, Any]:
...
{{
  ...
  "referral_pitch": "<a compelling 2-3 sentence referral pitch written in first-person candidate voice>"
}}"""
...
def analyze_resume_and_match_job(job: JobPosting, file_bytes: bytes, filename: str) -> Dict[str, Any]:
    resume_text = _extract_text(file_bytes, filename)
    api_key = settings.GEMINI_API_KEY or os.getenv("GEMINI_API_KEY", "")

    if not api_key:
        return _fallback_analysis(job, resume_text)

    return gemini_circuit_breaker.call(
        func=lambda: _call_gemini(api_key, job, resume_text),
        fallback_func=lambda: _fallback_analysis(job, resume_text),
    )
